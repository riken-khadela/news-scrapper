...
def get_google_search_results(search_term = "Funding & Investment", country_code='US'):
    try:
        ...
        query = f"{search_term} site:yourstory.com"
        params = {
            "q": query, "tbs": "qdr:w", 
...
def parse_google_results(html, key_data, tag, search_term):
    ...
    for a_tag in anchor_tags:
        skip_links = [
            "https://yourstory.com/topic/",
            'https://yourstory.com/category/'
        ]
        with open("story.html", '+w') as f:f.write(html)
        
        href = a_tag.get("href")
        if not href or "yourstory.com" not in href:
            continue
        elif href == "https://yourstory.com/" or href in skip_links or href in skip_links:
            continue
        elif href.startswith("/url?q") or href.startswith("/search?"):
            continue
        elif not "yourstory.com" in href:
            continue
